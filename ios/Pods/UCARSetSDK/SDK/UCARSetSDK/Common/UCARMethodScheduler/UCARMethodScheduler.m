...
@implementation UCARMethodScheduler

// 单例
+ (instancetype)sharedInstance {
    static UCARMethodScheduler *scheduler;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        scheduler = [[UCARMethodScheduler alloc] init];
    });
    return scheduler;
}
 ...
 */
- (nullable id)ucarmethod_executeMethodWithUrl:(NSURL *)url
{
    // 防止远程调用URL遭到黑客劫持，需要对URL的增加一些校验
    NSError *error = nil;
    if (nil == url.host || url.host.length > 255 || [url.host containsString:@"eval("])
    {
        error = CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"url里的target输入不合法");
    }
    
    if (nil == url.path || url.path.length > 255 || [url.path containsString:@"eval("])
    {
        error = CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"url里的action输入不合法");
    }
    
    if (url.query.length > 255 || [url.query containsString:@"eval("])
    {
        error = CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"url里的param输入不合法");
    }
    NSString *errormsg =[NSString stringWithFormat:@"UCARMethodScheduler Assert:%@",error];
    if (error) {
    ...
    return [self ucarmethod_executeInstanceMethod:NSSelectorFromString(methodName) inTarget:url.host params:params];
}
...
{
    @autoreleasepool{
        id result = [self ucarpay_processErrorWithMethod:method inTarget:target params:params];
        if ([result isKindOfClass:[NSError class]]) {
            NSString *errormsg =[NSString stringWithFormat:@"UCARMethodScheduler Assert:%@",(NSError *)result];
...
- (nullable id)ucarpay_processErrorWithMethod:(nonnull SEL)method inTarget:(nonnull id )target params:(nullable NSArray *)params{
    if (NULL == method) {
        return CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"请传入一个非空的选择子");
    }
    if (nil == target) {
        return CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"请传入一个非空的对象");
    }
    if (![target respondsToSelector:method]) {
        return CreateNSError(UCARMETHOD_ERROR_RES_RESPONSE,@"未被实现的方法");
    }
    NSMethodSignature* methodSig = [target methodSignatureForSelector:method];
    if(nil == methodSig) {
        return CreateNSError(UCARMETHOD_ERROR_RES_METHODSIG,@"未取到方法签名");
    }
    if ( ([methodSig numberOfArguments]-2) != params.count) {
        return CreateNSError(UCARMETHOD_ERROR_RES_ARGUMENTS,@"方法实际的参数个数，与params数组不对等！（提示：想传nil的地方用[NSNull null]代替）");
    };
    ...
    //    NSLog(@"UCARMethodScheduleraa：获取的方法签名为%@,返回值类型为%s",methodSig2,returnaa);
    
    const char* retType = [methodSig methodReturnType];
//    NSLog(@"UCARMethodScheduler：获取的方法签名为%@,返回值类型为%s",methodSig,retType);
    ...
    return CreateNSError(UCARMETHOD_ERROR_RES_RETTYPE,returnErrorStr);
}
...
- (nullable id)ucarmethod_executeClassMethod:(nonnull SEL)method inTarget:(nonnull char *)className params:(nullable NSArray *)params {
    ...
    return [self ucarmethod_executeInstanceMethod:method inTarget:class params:params];
}
...
- (nullable Class)getClassWithName:(nonnull char *)className
{
    NSError *error = nil;
    if (nil == className) {
        error = CreateNSError(UCARMETHOD_ERROR_RES_PARAM,@"入参不合法");
    }
    Class class = objc_getClass(className);
    if (Nil == class) {
        error = CreateNSError(UCARMETHOD_ERROR_RES_NILCLASS,@"未找到对应的类");
    }
    NSString *errormsg =[NSString stringWithFormat:@"UCARMethodScheduler Assert:%@",error];
    if (error) {
        NSAssert(nil != error,errormsg);
