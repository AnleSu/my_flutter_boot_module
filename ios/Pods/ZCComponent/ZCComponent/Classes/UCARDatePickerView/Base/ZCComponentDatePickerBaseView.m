...
@implementation ZCComponentDatePickerBaseView
...
- (UIView *)alertView {
    if (!_alertView) {
        _alertView = [[UIView alloc]initWithFrame:CGRectMake(0, SCREEN_HEIGHT - kTopViewHeight - kPickerHeight - UCAR_BOTTOM_MARGIN, SCREEN_WIDTH, kTopViewHeight + kPickerHeight + UCAR_BOTTOM_MARGIN)];
...
- (UIView *)topView {
    if (!_topView) {
        ...
        _topView.backgroundColor = kUCARToolBarColor;
        // 设置子视图的大小随着父视图变化
...
- (UIButton *)leftBtn {
    if (!_leftBtn) {
        ...
        _leftBtn.backgroundColor = kUCARToolBarColor;
        _leftBtn.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin;
        _leftBtn.titleLabel.font = [UIFont systemFontOfSize:17.0f * kScaleFit];
        [_leftBtn setTitleColor:kCanclePickViewColor forState:UIControlStateNormal];
...
- (UIButton *)rightBtn {
    if (!_rightBtn) {
        ...
        _rightBtn.backgroundColor = kUCARToolBarColor;
        _rightBtn.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleLeftMargin;
        _rightBtn.titleLabel.font = [UIFont systemFontOfSize:17.0f * kScaleFit];
...
- (UIView *)lineView {
    if (!_lineView) {
        ...
        _lineView.backgroundColor = UCAR_RGB_HEX(0xf1f1f1, 1.0f);
