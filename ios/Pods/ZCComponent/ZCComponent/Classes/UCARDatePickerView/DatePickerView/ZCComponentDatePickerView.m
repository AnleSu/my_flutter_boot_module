//
//  UCARDatePickerView.m
//  UCARPickerViewDemo
//
//  Created by szq on 2019/4/25.
//  Copyright © 2019年 ucar. All rights reserved.//

#import "ZCComponentDatePickerView.h"
#import "ZCComponentPickerViewMacro.h"

/// 时间选择器的类型
typedef NS_ENUM(NSInteger, UCARDatePickerStyle) {
    UCARDatePickerStyleSystem,   // 系统样式：使用 UIDatePicker 类
    UCARDatePickerStyleCustom    // 自定义样式：使用 UIPickerView 类
};

@interface ZCComponentDatePickerView ()<UIPickerViewDataSource, UIPickerViewDelegate>
{
...
/** 显示类型 */
@property (nonatomic, assign) UCARDatePickerMode showType;
/** 时间选择器的类型 */
@property (nonatomic, assign) UCARDatePickerStyle style;
/** 限制最小日期 */
...
/** 选中后的回调 */
@property (nonatomic, copy) UCARDateResultBlock resultBlock;
/** 取消选择的回调 */
@property (nonatomic, copy) UCARDateCancelBlock cancelBlock;

@end

@implementation ZCComponentDatePickerView
...
+ (void)showDatePickerWithTitle:(NSString *)title
    ...
    ZCComponentDatePickerView *datePickerView = [[ZCComponentDatePickerView alloc]initWithTitle:title dateType:dateType defaultSelValue:defaultSelValue minDate:minDate maxDate:maxDate isAutoSelect:isAutoSelect themeColor:themeColor resultBlock:resultBlock cancelBlock:cancelBlock];
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self = [super init]) {
        ...
        if (minDate) {
            ...
            if (self.showType == UCARDatePickerModeTime || self.showType == UCARDatePickerModeCountDownTimer || self.showType == UCARDatePickerModeHM) {
                self.minLimitDate = [NSDate ucar_setHour:0 minute:0];
            } else if (self.showType == UCARDatePickerModeMDHM) {
                self.minLimitDate = [NSDate ucar_setMonth:1 day:1 hour:0 minute:0];
            } else if (self.showType == UCARDatePickerModeMD) {
                self.minLimitDate = [NSDate ucar_setMonth:1 day:1];
            } else {
        ...
        if (maxDate) {
            ...
            if (self.showType == UCARDatePickerModeTime || self.showType == UCARDatePickerModeCountDownTimer || self.showType == UCARDatePickerModeHM) {
                self.maxLimitDate = [NSDate ucar_setHour:23 minute:59];
            } else if (self.showType == UCARDatePickerModeMDHM) {
                self.maxLimitDate = [NSDate ucar_setMonth:12 day:31 hour:23 minute:59];
            } else if (self.showType == UCARDatePickerModeMD) {
                self.maxLimitDate = [NSDate ucar_setMonth:12 day:31];
            } else {
                self.maxLimitDate = [NSDate distantFuture]; // 遥远的未来的一个时间点
            }
        }
        BOOL minMoreThanMax = [self.minLimitDate ucar_compare:self.maxLimitDate format:self.selectDateFormatter] == NSOrderedDescending;
        ...
        if (defaultSelValue && defaultSelValue.length > 0) {
            NSDate *defaultSelDate = [NSDate ucar_getDate:defaultSelValue format:self.selectDateFormatter];
            if (!defaultSelDate) {
                UCARErrorLog(@"参数格式错误！参数 defaultSelValue 的正确格式是：%@", self.selectDateFormatter);
                NSAssert(defaultSelDate, @"参数格式错误！请检查形参 defaultSelValue 的格式");
                defaultSelDate = [NSDate date]; // 默认值参数格式错误时，重置/忽略默认值，防止在 Release 环境下崩溃！
            }
            if (self.showType == UCARDatePickerModeTime || self.showType == UCARDatePickerModeCountDownTimer || self.showType == UCARDatePickerModeHM) {
                self.selectDate = [NSDate ucar_setHour:defaultSelDate.ucar_hour minute:defaultSelDate.ucar_minute];
            } else if (self.showType == UCARDatePickerModeMDHM) {
                self.selectDate = [NSDate ucar_setMonth:defaultSelDate.ucar_month day:defaultSelDate.ucar_day hour:defaultSelDate.ucar_hour minute:defaultSelDate.ucar_minute];
            } else if (self.showType == UCARDatePickerModeMD) {
                self.selectDate = [NSDate ucar_setMonth:defaultSelDate.ucar_month day:defaultSelDate.ucar_day];
            } else {
        ...
        BOOL selectLessThanMin = [self.selectDate ucar_compare:self.minLimitDate format:self.selectDateFormatter] == NSOrderedAscending;
        BOOL selectMoreThanMax = [self.selectDate ucar_compare:self.maxLimitDate format:self.selectDateFormatter] == NSOrderedDescending;
        ...
        NSLog(@"最小时间：%@", [NSDate ucar_getDateString:self.minLimitDate format:self.selectDateFormatter]);
        NSLog(@"默认时间：%@", [NSDate ucar_getDateString:self.selectDate format:self.selectDateFormatter]);
        NSLog(@"最大时间：%@", [NSDate ucar_getDateString:self.maxLimitDate format:self.selectDateFormatter]);
#endif
        
        if (self.style == UCARDatePickerStyleCustom) {
            [self initDefaultDateArray];
        }
        [self initUI];
        
        // 默认滚动的行
        if (self.style == UCARDatePickerStyleSystem) {
            [self.datePicker setDate:self.selectDate animated:NO];
        } else if (self.style == UCARDatePickerStyleCustom) {
...
- (void)setupSelectDateFormatter:(UCARDatePickerMode)model {
    switch (model) {
        case UCARDatePickerModeTime:
        {
            self.selectDateFormatter = @"HH:mm";
            self.style = UCARDatePickerStyleSystem;
            _datePickerMode = UIDatePickerModeTime;
        }
            break;
        case UCARDatePickerModeDate:
        {
            ...
            break;
        case UCARDatePickerModeDateAndTime:
        {
            ...
            break;
        case UCARDatePickerModeCountDownTimer:
        {
            self.selectDateFormatter = @"HH:mm";
            self.style = UCARDatePickerStyleSystem;
        ...
        case UCARDatePickerModeYMDHM:
        {
            self.selectDateFormatter = @"yyyy/MM/dd HH:mm";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeMDHM:
        {
            self.selectDateFormatter = @"MM/dd HH:mm";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeYMD:
        {
            self.selectDateFormatter = @"yyyy/MM/dd";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeYM:
        {
            self.selectDateFormatter = @"yyyy/MM";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeY:
        {
            self.selectDateFormatter = @"yyyy";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeMD:
        {
            self.selectDateFormatter = @"MM/dd";
            self.style = UCARDatePickerStyleCustom;
        }
            break;
        case UCARDatePickerModeHM:
        {
            self.selectDateFormatter = @"HH:mm";
            self.style = UCARDatePickerStyleCustom;
        }
...
- (void)initDefaultDateArray {
    ...
    [self setupYearArr];
    // 2.设置 monthArr 数组
    [self setupMonthArr:self.selectDate.ucar_year];
    // 3.设置 dayArr 数组
    [self setupDayArr:self.selectDate.ucar_year month:self.selectDate.ucar_month];
    // 4.设置 hourArr 数组
    [self setupHourArr:self.selectDate.ucar_year month:self.selectDate.ucar_month day:self.selectDate.ucar_day];
    // 5.设置 minuteArr 数组
    [self setupMinuteArr:self.selectDate.ucar_year month:self.selectDate.ucar_month day:self.selectDate.ucar_day hour:self.selectDate.ucar_hour];
    // 根据 默认选择的日期 计算出 对应的索引
    _yearIndex = self.selectDate.ucar_year - self.minLimitDate.ucar_year;
    _monthIndex = self.selectDate.ucar_month - ((_yearIndex == 0) ? self.minLimitDate.ucar_month : 1);
    _dayIndex = self.selectDate.ucar_day - ((_yearIndex == 0 && _monthIndex == 0) ? self.minLimitDate.ucar_day : 1);
    _hourIndex = self.selectDate.ucar_hour - ((_yearIndex == 0 && _monthIndex == 0 && _dayIndex == 0) ? self.minLimitDate.ucar_hour : 0);
    _minuteIndex = self.selectDate.ucar_minute - ((_yearIndex == 0 && _monthIndex == 0 && _dayIndex == 0 && _hourIndex == 0) ? self.minLimitDate.ucar_minute : 0);
...
- (void)setupYearArr {
    ...
    for (NSInteger i = self.minLimitDate.ucar_year; i <= self.maxLimitDate.ucar_year; i++) {
...
- (void)setupMonthArr:(NSInteger)year {
    ...
    if (year == self.minLimitDate.ucar_year) {
        startMonth = self.minLimitDate.ucar_month;
    }
    if (year == self.maxLimitDate.ucar_year) {
        endMonth = self.maxLimitDate.ucar_month;
    }
...
- (void)setupDayArr:(NSInteger)year month:(NSInteger)month {
    ...
    NSInteger endDay = [NSDate ucar_getDaysInYear:year month:month];
    if (year == self.minLimitDate.ucar_year && month == self.minLimitDate.ucar_month) {
        startDay = self.minLimitDate.ucar_day;
    }
    if (year == self.maxLimitDate.ucar_year && month == self.maxLimitDate.ucar_month) {
        endDay = self.maxLimitDate.ucar_day;
    }
    NSMutableArray *tempArr = [NSMutableArray array];
...
- (void)setupHourArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day {
    ...
    if (year == self.minLimitDate.ucar_year && month == self.minLimitDate.ucar_month && day == self.minLimitDate.ucar_day) {
        startHour = self.minLimitDate.ucar_hour;
    }
    if (year == self.maxLimitDate.ucar_year && month == self.maxLimitDate.ucar_month && day == self.maxLimitDate.ucar_day) {
        endHour = self.maxLimitDate.ucar_hour;
    }
...
- (void)setupMinuteArr:(NSInteger)year month:(NSInteger)month day:(NSInteger)day hour:(NSInteger)hour {
    ...
    if (year == self.minLimitDate.ucar_year && month == self.minLimitDate.ucar_month && day == self.minLimitDate.ucar_day && hour == self.minLimitDate.ucar_hour) {
        startMinute = self.minLimitDate.ucar_minute;
    }
    if (year == self.maxLimitDate.ucar_year && month == self.maxLimitDate.ucar_month && day == self.maxLimitDate.ucar_day && hour == self.maxLimitDate.ucar_hour) {
        endMinute = self.maxLimitDate.ucar_minute;
    }
...
- (void)scrollToSelectDate:(NSDate *)selectDate animated:(BOOL)animated {
    ...
    NSInteger yearIndex = selectDate.ucar_year - self.minLimitDate.ucar_year;
    NSInteger monthIndex = selectDate.ucar_month - ((yearIndex == 0) ? self.minLimitDate.ucar_month : 1);
    NSInteger dayIndex = selectDate.ucar_day - ((yearIndex == 0 && monthIndex == 0) ? self.minLimitDate.ucar_day : 1);
    NSInteger hourIndex = selectDate.ucar_hour - ((yearIndex == 0 && monthIndex == 0 && dayIndex == 0) ? self.minLimitDate.ucar_hour : 0);
    NSInteger minuteIndex = selectDate.ucar_minute - ((yearIndex == 0 && monthIndex == 0 && dayIndex == 0 && hourIndex == 0) ? self.minLimitDate.ucar_minute : 0);
    
    NSArray *indexArr = [NSArray array];
    if (self.showType == UCARDatePickerModeYMDHM) {
        indexArr = @[@(yearIndex), @(monthIndex), @(dayIndex), @(hourIndex), @(minuteIndex)];
    } else if (self.showType == UCARDatePickerModeMDHM) {
        indexArr = @[@(monthIndex), @(dayIndex), @(hourIndex), @(minuteIndex)];
    } else if (self.showType == UCARDatePickerModeYMD) {
        indexArr = @[@(yearIndex), @(monthIndex), @(dayIndex)];
    } else if (self.showType == UCARDatePickerModeYM) {
        indexArr = @[@(yearIndex), @(monthIndex)];
    } else if (self.showType == UCARDatePickerModeY) {
        indexArr = @[@(yearIndex)];
    } else if (self.showType == UCARDatePickerModeMD) {
        indexArr = @[@(monthIndex), @(dayIndex)];
    } else if (self.showType == UCARDatePickerModeHM) {
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (_isAutoSelect) {
        // 滚动完成后，执行block回调
        if (self.resultBlock) {
            NSString *selectDateValue = [NSDate ucar_getDateString:self.selectDate format:self.selectDateFormatter];
...
- (NSDate *)getDidSelectedDate:(NSInteger)component row:(NSInteger)row {
    ...
    return [NSDate ucar_getDate:selectDateValue format:self.selectDateFormatter];
}
...
- (void)showWithAnimation:(BOOL)animation {
    ...
    if (animation) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            CGRect rect = self.alertView.frame;
            rect.origin.y -= kPickerHeight + kTopViewHeight + UCAR_BOTTOM_MARGIN;
            self.alertView.frame = rect;
        }];
    }
}
