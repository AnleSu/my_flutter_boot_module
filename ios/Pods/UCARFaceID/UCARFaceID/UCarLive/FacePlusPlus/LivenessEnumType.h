...
typedef enum MGLivenessDetectionFailedType {
    ...
    DETECTION_FAILED_TYPE_NOTVIDEO,                 ///< 发现使用者在使用非连续的图像进行活体检测
    DETECTION_FAILED_TYPE_TIMEOUT,                  ///< 检测超时
    DETECTION_FAILED_TYPE_FACELOSTNOTCONTINUOUS,    ///< 人脸时不时丢失，被算法判定为非连续  (不适合提示用户)
    DETECTION_FAILED_TYPE_TOOMANYFACELOST,          ///< 人脸从拍摄区域消失时间过长 (不适合提示用户)
    DETECTION_FAILED_TYPE_FACENOTCONTINUOUS,        ///< 人脸动作过快导致非连续  (不适合提示用户)
    DETECTION_FAILED_TYPE_MASK,                     ///< 面具攻击  (不适合提示用户)
    
    
    DETECTION_FAILED_TYPE_CANCEL,                    ///< 取消
    DETECTION_FAILED_TYPE_CAMERA                    ///< 摄像头权限失败
