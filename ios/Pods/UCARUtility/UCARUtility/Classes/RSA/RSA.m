...
+ (NSData *)stripPublicKeyHeader:(NSData *)d_key {
    ...
    static unsigned char seqiod[] = {0x30, 0x0d, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86,
...
+ (NSData *)encryptData:(NSData *)data withKeyRef:(SecKeyRef)keyRef isSign:(BOOL)isSign {
    ...
    for (int idx = 0; idx < srclen; idx += src_block_size) {
        // UCARLoggerDebug(@"%d/%d block_size: %d", idx, (int)srclen, (int)block_size);
        ...
        if (status != 0) {
            UCARLoggerDebug(@"SecKeyEncrypt fail. Error Code: %d", status);
            ret = nil;
            break;
        } else {
